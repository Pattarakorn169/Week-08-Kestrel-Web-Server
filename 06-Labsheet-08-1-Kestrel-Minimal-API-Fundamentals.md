...
2. กลับไปที่หน้าเบราว์เซอร์แล้วกดปุ่ม **Refresh (F5)** สังเกตว่าเกิดอะไรขึ้น และอธิบายสั้นๆ ว่าทำไมจึงเป็นเช่นนั้น
   - **คำตอบ**
   ```
     Failed to Load Page เพราะกดหยุดโปรแกรมเซิร์ฟเวอรืทำให้ตัวโปรแกรมหยุดรัน
   ```
---
...
```
[09:00:21] LED Control: off
[09:00:23] LED Control: on
```
...
```
   builder ทำหน้าที่สร้างและกำหนดค่า Web Application ส่วน app ทำหน้าที่เป็นตัว Application ที่รับและตอบสนอง HTTP Request
```
...
```
	.NET Minimal API ตั้งค่าและพัฒนาได้ง่าย รวดเร็ว เหมาะกับ IoT Gateway และใช้ทรัพยากรไม่มาก ส่วน LAMP ติดตั้งและใช้งานระบบเว็บแบบดั้งเดิมได้สะดวก แต่มีขั้นตอนและส่วนประกอบมากกว่า
```
3. นักศึกษาคิดว่าการเพิ่ม `/api/` เข้าไปใน route นั้นมีประโยชน์อย่างไรบ้าง ถ้าไม่ใส่จะเกิดปัญหาอะไรบ้าง
```
	การใส่ /api/ ช่วยแยกเส้นทาง API ให้ชัดเจน จัดการและขยายระบบง่ายขึ้น หากไม่ใส่อาจทำให้ route ของหน้าเว็บและ API สับสนหรือชนกันได้
```
